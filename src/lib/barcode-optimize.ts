...
async function loadToCanvas(src: HTMLImageElement | Blob | string): Promise<HTMLCanvasElement> {
  const img = await (async (): Promise<HTMLImageElement> => {
...
function upscaleNearest(canvas: HTMLCanvasElement, minHeight: number = 180): HTMLCanvasElement {
...
// Otsu binarization
function binarizeOtsu(canvas: HTMLCanvasElement): HTMLCanvasElement {
  const { width: w, height: h } = canvas;
  const ctx = canvas.getContext('2d', { willReadFrequently: true });
  if (!ctx) throw new Error('Could not get 2d context');
...
function median1x3(canvas: HTMLCanvasElement): HTMLCanvasElement {
  const { width: w, height: h } = canvas;
  const ctx = canvas.getContext('2d', { willReadFrequently: true });
  if (!ctx) throw new Error('Could not get 2d context');
...
function addQuietZone(canvas: HTMLCanvasElement, pad: number = 24): HTMLCanvasElement {
  const out = document.createElement('canvas');
...
export function binaryBitmapFromCanvas(canvas: HTMLCanvasElement, useGlobal: boolean = false): BinaryBitmap {
  const ctx = canvas.getContext('2d', { willReadFrequently: true });
  if (!ctx) throw new Error('Could not get 2d context');
...
// Full pipeline
export async function prepForZXingImage(src: HTMLImageElement | Blob | string): Promise<HTMLCanvasElement> {
...
export function prepForZXing(c: HTMLCanvasElement): HTMLCanvasElement {
  c = autoCrop(c);
  c = upscaleNearest(c, 200);   // try 200–300 px tall
  c = binarizeOtsu(c);
  c = median1x3(c);              // optional
